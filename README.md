...
## 11. Каталог паник и ошибок компилятора (`troll_die`)
...
| `TR_GBSV_SYNTAX` | 1 | Нарушение 11-байтного формата терминатора `:[...|...|...];...}` или отсутствие пробела 0x20 |
| `TR_GBSV_B64` | 1 | Несовпадение Base64-проекции $\sigma_{b64}$ с побитовым сдвигом нечетных гармоник |
| `TR_GBSV_GF` | 1 | Несовпадение синдрома поля Галуа $\mathbb{F}_{2^8}$ ($\chi_{gf}$) по неприводимому полиному AES |
| `TR_GBSV_TUX` | 1 | Нарушение $p$-адического баланса $\tau_{tux}$ ($\nu_2(W)$ vs $\nu_3(W)$) взвешенных байтов префикса |
| `TR_GBSV_ZETA` | 1 | Ошибка квантового коллапса Хэмминга $\zeta$ относительно предыдущей строки/затравки |
...
Ортогональные модификаторы:
  --gbsv              Активация криптографической верификации строк Galois-Base64 (GBSV)
