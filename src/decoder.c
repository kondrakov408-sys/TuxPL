...
DecodedInstruction decode_instruction(
    ...
    uint8_t age = tux_cell_age(val);
    uint8_t type_tag = tux_cell_type(val);
    ...
    /* 3. Первичный вход */
    uint16_t c_in = (uint16_t)(val & 0xFFFF);

    /* 4. Синтез динамической ширины текущей инструкции W_t */
    uint64_t w_mix = tux_crazy64((uint64_t)val, genome->chromosomes[1] ^ (uint64_t)prev_width);
    ...
    int apply_shift = is_apocalypse && (type_tag != TUX_TYPE_OPCODE);
    uint64_t shift = apply_shift ? (ctx_mix % TUX_NUM_OPCODES) : 0;

    /* 6. Возрастные домены декодирования опкода */
    uint64_t raw_op = 0;
    if (age == TUX_AGE_YOUNG) {
        raw_op = (c_in + shift) % TUX_NUM_OPCODES;
    } else if (age == TUX_AGE_ADULT) {
        raw_op = (c_in + shift + genome->chromosomes[1]) % TUX_NUM_OPCODES;
    } else { /* TUX_AGE_OLD */
        raw_op = tux_crazy64(c_in + shift, (uint64_t)age ^ genome->chromosomes[2]) % TUX_NUM_OPCODES;
    }
    res.op = (Opcode)raw_op;

    /* 7. Маскирование операнда шириной prev_width */
    int64_t raw_arg = (int64_t)val;
    if (prev_width == 1) {
